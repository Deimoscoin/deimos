...
###   deimoscore.org and deimos.org and compares them.
...
WORKINGDIR="/tmp/deimos_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://deimoscore.org"
HOST2="https://deimoscoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="deimos-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'deimos-',
...
if [ $? -ne 0 ]; then
   echo "deimos.org failed to provide signature file, but deimoscore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "deimos.org and deimoscore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the DeiMos Core binary release signing key installed?"
   fi
