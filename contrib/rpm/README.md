...
The RPM spec file provided here is for DeiMos-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://deimos.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[DeiMos git repository](https://github.com/deimos/deimos) but are not part of
...
Currently only `Source10` is used, for the example `deimos.conf` file.
...
in the deimos git repository. At present only an SVG version of the DeiMos
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the deimos
...
needed when 0.12.1 is released, a proper fix is already in the DeiMos git
master branch.
...
idea to build DeiMos Core against a static version of that library compiled
...
The `deimos.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `deimos-server` package. At this point, most RPM based
...
The `deimos-server` package should have SELinux support. How to properly do
...
DeiMos-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/deimos/). He has been
packaging DeiMos for Fedora at least since 2012.
...
4. I renamed the `deimos` package that contains the Qt GUI to `deimos-core` as
that appears to be how the general population refers to it, in contrast to
`deimos-xt` or `deimos-classic`. I wanted to make sure the general population
