#!/bin/bash
# Copyright (c) 2013 The DeiMos Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete DeiMos database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no DeiMos datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old DeiMos datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected DeiMos 0.7 datadir."
    ;;
  3)
    echo "Detected DeiMos pre-0.8 datadir."
    ;;
  4)
    echo "Detected DeiMos 0.8 datadir."
    ;;
esac
