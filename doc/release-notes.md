DeimOS Core version 0.17.1 is now available from:

  <https://deimoscore.org/bin/deimos-core-0.17.1/>
...
Please report bugs using the issue tracker at GitHub:
  ...
  <https://github.com/deimos/deimos/issues>

To receive security and update notifications, please subscribe to:

  <https://deimoscore.org/en/list/announcements/join/>
...
DeimOS Core has supported automatically pruning the blockchain since 0.11. Pruning
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/deimos/deimos/blob/master/doc/zmq.md) for
...
When DeimOS Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `deimos-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`deimos-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/deimos-cli -named help command="help"
    src/deimos-cli -named getblockhash height=0
    src/deimos-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/deimos-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
The Alert System was [disabled and deprecated](https://deimos.org/en/alert/2016-11-01-alert-retirement) in DeimOS Core 0.12.1 and removed in 0.13.0. 
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   about the memory usage of DeimOS Core. This was added in conjunction with
   optimizations to memory management. See [Pull #8753](https://github.com/deimos/deimos/pull/8753)
...
installer (on Windows) or just copy over `/Applications/DeimOS-Qt` (on Mac)
or `deimosd`/`deimos-qt` (on Linux).
...
Compatibility
==============
DeimOS Core is extensively tested on multiple operating systems using
...
DeimOS Core should also work on most other Unix-like systems but is not
...
DeimOS Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
deimos data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile DeimOS Core is now 4.8. No effort will be
...
GUI changes
-----------
- Uses of "µDEI" in the GUI now also show the more colloquial term "bits", specified in BIP176.
...
- When deimos is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If deimos is started
...
- When running deimosd with `-debug` but without `-daemon`, logging to stdout
...
When deimos is configured with the `-deprecatedrpc=accounts` setting, specifying
...
- #14698 `ec71f06` Add deimos-tx.exe into Windows installer (ken2812221)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/deimos/).
