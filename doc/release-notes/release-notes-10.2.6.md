DeimOS Core version 10.2.6 is now available from:

  <https://deimoscoin.org/#downloads>
...
Please report bugs using the issue tracker at GitHub:
  ...
  <https://github.com/deimoscoin/deimos/issues>
...
- deimos-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- deimos-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has been changed to 18443 to avoid conflict with testnet's default of 18332.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest deimos.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.deimos.sprovoost.nl (Sjors)
...
- #11970 `b7450cd` Add test coverage for deimos-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to deimos-cli to allow RPC password to be read from standard input (jharvell)
...
- #11621 `1f7695b` [build] Add temp_deimos_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` deimos_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.deimos doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for deimos-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in deimos-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop deimosd (OmeGak)
...
- #10529 `331352f` Improve deimosd systemd service file (Flowdalic)
