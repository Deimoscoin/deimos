...
*Setup instructions for a Gitian build of DeiMos Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the DeiMos
...
- [Building DeiMos Core](#building-deimos-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-deimos-host/README.md).
...
Clone the git repositories for deimos and Gitian.
...
in the deimos repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building DeiMos Core
----------------

To build DeiMos Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the deimos repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/deimos/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/deimos/deimos
...
For example:
```bash
URL=https://github.com/laanwj/deimos.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit deimos=${COMMIT} --url deimos=${URL} ../deimos/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit deimos=${COMMIT} --url deimos=${URL} ../deimos/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit deimos=${COMMIT} --url deimos=${URL} ../deimos/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the deimos git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../deimos/contrib/gitian-descriptors/*|sort|uniq )
...
DEIPATH=/some/root/path/deimos
SIGPATH=/some/root/path/deimos-detached-sigs

./bin/gbuild --url deimos=${DEIPATH},signature=${SIGPATH} ../deimos/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/deimos-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/deimos-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/deimos-osx-build.assert
```
...
[deimos-core/gitian.sigs](https://github.com/deimos-core/gitian.sigs/) repository, or if that's not possible create a pull
