This directory contains the DeimOSQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the DeimOS UI.
...
ClientModel is used by the main application `deimosgui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/deimos/deimos/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a deimos:... link from a browser).
...
* UI elements like DeimOSAmountField, which inherit from QWidget.
* `deimosstrings.cpp`: automatically generated
* `deimosunits.(h/cpp)`: DEI / mDEI / etc handling
* `callback.h`
...
* don't change `local/deimos_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "deimos-qt" under "Run", which is where you can also add command line arguments)
