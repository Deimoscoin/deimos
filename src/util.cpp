...
const char * const DEIMOS_CONF_FILENAME = "deimos.conf";
const char * const DEIMOS_PID_FILENAME = "deimosd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "deimosd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DeimOS
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DeimOS
    // Mac: ~/Library/Application Support/DeimOS
    // Unix: ~/.deimos
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DeimOS";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DeimOS";
#else
    // Unix
    return pathRet / ".deimos";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", DEIMOS_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", DEIMOS_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure DeimOS Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("DeimOS Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The DeimOS Core developers";
    }
