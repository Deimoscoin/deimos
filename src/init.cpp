...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), DEIMOS_CONF_FILENAME));
    if (mode == HMM_DEIMOSD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), DEIMOS_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/deimoscoin/deimos>";
    const std::string URL_WEBSITE = "<https://deimoscoin.org>";

    return CopyrightHolders(strprintf(_("Copyright (C) %i-%i"), 20017, COPYRIGHT_YEAR) + " ") + "\n" +
           "\n" +
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(GetArg("-conf", DEIMOS_CONF_FILENAME)).string());
