...
unsigned int GetNextWorkRequiredV1(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params, int algo)
{
	...
	// DeiMos: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params, int algo)
{
	if (pindexLast->nHeight < params.multiAlgoDiffChangeTarget)
	  ...
	  else if(pindexLast->nHeight < params.DEIComputationRewardD)
                return GetNextWorkRequiredV4(pindexLast, pblock, params, algo);
	  else if(pindexLast->nHeight < params.DEIComputationRewardD2)
                return GetNextWorkRequiredV4(pindexLast, pblock, params, algo);
	 else if(pindexLast->nHeight < params.DEIComputationRewardD3)
                return GetNextWorkRequiredV4(pindexLast, pblock, params, algo);
	else
		return GetNextWorkRequiredV4(pindexLast, pblock, params, algo);
}
