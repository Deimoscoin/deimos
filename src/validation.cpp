...
#if defined(NDEBUG)
# error "DeiMos cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "DeiMos Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s DEI additional fees, %d delta bytes\n",
...
CAmount GetDEISubsidy(int nHeight, const Consensus::Params& consensusParams) {
	...
	else if(nHeight<consensusParams.workComputationChangeTarget)
	{
		...
		//decrease reward by 3% every month
		for(int i = 0; i < weeks; i++)  qSubsidy -= (qSubsidy/300);
	}

	//New DEI Hard fork on block 333238
	else if(nHeight<consensusParams.DEIComputationRewardD)
        {
                qSubsidy = 1079*COIN;
                int blocks = nHeight - consensusParams.DEIComputationRewardD;
                int weeks = (blocks / consensusParams.patchBlockRewardDuration2)+1;
                //decrease reward by 3% every month
                for(int i = 0; i < weeks; i++)  qSubsidy -= (qSubsidy/300);
        }
	
	else if(nHeight<consensusParams.DEIComputationRewardD2)
        {
                qSubsidy = 240*COIN/2;
                int blocks = nHeight - consensusParams.DEIComputationRewardD2;
                int weeks = (blocks / consensusParams.patchBlockRewardDuration2)+1;
                //decrease reward by 3% every month
                for(int i = 0; i < weeks; i++)  qSubsidy -= (qSubsidy/300);
        }
	...
	else
	{
		...
		int64_t weeks = (blocks / consensusParams.patchBlockRewardDuration2)+1;
		for(int64_t i = 0; i < weeks; i++) qSubsidy -= (qSubsidy/200);
		}
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
	...
	if(nHeight < consensusParams.nDiffChangeTarget) {
		...
		if(nHeight < 72759)  //72759
		{
			nSubsidy = 1079 * COIN;
		}
		else if(nHeight < 262846)  //262846
		{
			nSubsidy = 300 * COIN;
		}
		else if(nHeight < 318143)  //318143
                {
                        nSubsidy = 1067 * COIN;
                }
		else if(nHeight < 333238)  //333238
                {
                        nSubsidy = 1079 * COIN;
                }
		else if(nHeight < 333238)  //393670
                {
                        nSubsidy = 120 * COIN;
                }
		...
		nSubsidy = GetDEISubsidy(nHeight, consensusParams);
	}

	//make sure the reward is at least 1 DEI
	if(nSubsidy < COIN) {
		nSubsidy = COIN;
	}
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    if (VersionBitsState(pindex->pprev, chainparams.GetConsensus(), Consensus::DEPLOYMENT_NVERSIONBIPS, versionbitscache) == THRESHOLD_ACTIVE) {
